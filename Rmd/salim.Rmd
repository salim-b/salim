...
```{r, purl = FALSE}
regex_text_normalization <-
  tibble::tibble(category = character(),
                 purpose = character(),
                 pattern = list(),
                 replacement = character()) %>%
  # FOKUS Aargau
  tibble::add_row(category = "fa",
                  purpose = "standardize spelling of the _FOKUS Aargau_ project title",
                  pattern = list("Fokus(?=[ -]Aargau)"),
                  replacement = "FOKUS") %>%
  # harmonize punctuation
  tibble::add_row(category = "harmonize_punctuation",
  ...
  tibble::add_row(category = "prettify_punctuation",
                  purpose = "use narrow non-breaking space between numbers and percentage signs",
                  pattern = list("(\\d+) ?(\\\\?%|Prozent|percent|per cent)(?![\"'])"),
                  replacement = "\\1\u202f\\2") %>%
  tibble::add_row(category = "prettify_punctuation",
                  purpose = "use narrow non-breaking space between characters of common abbreviations",
                  pattern = list("(?i)(z\\.) ?(B\\.)", "(u\\.) ?(a\\.)", "(d\\.) ?(h\\.)"),
                  replacement = "\\1\u202f\\2") %>%
  tibble::add_row(category = "prettify_punctuation",
