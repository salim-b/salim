...
#' Phrase date(time)s
#'
#' Formats date(time)s as strings according to the specified `format`s and `lang`, applying some typographic tweaks. A convenience wrapper around
...
#' @param lang Identifier of the language to phrase `x` in according to [ISO 639-1](https://en.wikipedia.org/wiki/ISO_639-1) or
#'   [ISO 639-3](https://en.wikipedia.org/wiki/ISO_639-3) with an optional country identifier separated by a hyphen (`-`). A character scalar.
#'
...
phrase_datetime <- function(x,
  ...
  if (length(ix_date) > 0L) {
    result[ix_date] %<>% pal::when(startsWith(lang, "en") ~ stringr::str_replace(string = .[ix_date],
                                                                                 pattern = "(\\w+)\\s+(\\d+)",
                                                                                 replacement = "\\1\u00a0\\2"),
                                   startsWith(lang, "de") ~ stringr::str_replace(string = .[ix_date],
                                                                                 pattern = "(\\d+\\.)\\s+",
                                                                                 replacement = "\\1\u00a0"),
                                   startsWith(lang, "fr") ~ stringr::str_replace(string = .[ix_date],
                                                                                 pattern = "(\\d+)\\s+",
                                                                                 replacement = "\\1\u00a0"),
                                   ~ .)
  }
