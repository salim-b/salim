...
quarto_chunks <- function(data,
  ...
  if (data$label |>
      ...
      stringr::str_subset(pattern = "\\{[^{].*?\\}",
                          negate = TRUE) |>
      anyDuplicated()) {
    cli::cli_abort("Labels in column {.var label} must be unique.")
  }
  ...
  ## check lbls and itr_vars congruency
  has_itr_lbl <- stringr::str_detect(string = data$label,
                                     pattern = "\\{[^{].*?\\}")
  has_itr_col <- !purrr::map_lgl(data$itr_vars,
                                 is.null)
  if (any(has_itr_col & !has_itr_lbl)) {
    cli::cli_abort("All iterated {.arg data} entries must use variables via glue string interpolation in their {.var label}.")
  }
  if (any(has_itr_lbl & !has_itr_col)) {
    cli::cli_abort("All data entries with a {.arg label} to be iterated must have proper {.var itr_vars} set.")
  }
