...
quarto_fig_chunk <- function(body,
  ...
  glue::glue(paste0(c("```{{r}}",
                      "#| label: {yaml::as.yaml(label)}",
                      "#| fig-cap:",
                      as_code_chunk_array(fig_cap),
                      "#| fig-subcap:"[has_fig_subcap],
                      fig_subcap,
                      "#| fig-column: {layout_class}",
                      "#| fig-width: {yaml::as.yaml(fig_width)}"[!is.null(fig_width)],
                      "#| fig-height: {yaml::as.yaml(fig_height)}"[!is.null(fig_height)],
                      "#| fig-pos: {fig_pos}"[!is.null(fig_pos)],
                      "#| fig-link: {yaml::as.yaml(fig_link)}"[length(fig_link) > 0L],
                      "#| out-width: {yaml::as.yaml(out_width)}"[!is.null(out_width)],
                      "#| out-height: {yaml::as.yaml(out_height)}"[!is.null(out_height)],
                      "",
                      "{body}",
                      "```"),
                    collapse = "\n"))
